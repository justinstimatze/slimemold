...
func FormatHookFindings(topo *types.Topology, vulns *types.Vulnerabilities, newClaims, newEdges, maxFindings int) string {
	...
	fmt.Fprintf(&b, "SLIMEMOLD [%s] — %d claims, %d edges", topo.Project, topo.ClaimCount, topo.EdgeCount)
	if newClaims > 0 || newEdges > 0 {
		fmt.Fprintf(&b, " (+%d new, +%d edges)", newClaims, newEdges)
	}
	b.WriteString("\n")
	...
	for _, f := range findings {
		...
		claimID := ""
		if len(v.ClaimIDs) > 0 {
			claimID = v.ClaimIDs[0]
		}

		switch v.Type {
		case "load_bearing_vibes":
			fmt.Fprintf(&b, "CHALLENGE %s [id:%s]\n", v.Description, claimID)
			b.WriteString("  → This is load-bearing with a weak basis — defend it, source it, or flag it as speculative.\n\n")
		case "fluency_trap":
			fmt.Fprintf(&b, "CHALLENGE %s [id:%s]\n", v.Description, claimID)
			b.WriteString("  → High confidence on weak basis — the fluency of the claim may be substituting for actual evidence.\n\n")
		case "unchallenged_chain":
			fmt.Fprintf(&b, "INVESTIGATE %s\n", v.Description)
			b.WriteString("  → No claim in this chain was questioned. Probe the weakest link.\n\n")
		case "coverage_imbalance":
			fmt.Fprintf(&b, "REBALANCE %s\n", v.Description)
			b.WriteString("  → Reasoning effort is unevenly distributed. Check if you're following the interesting gradient instead of the important one.\n\n")
		case "bottleneck":
			fmt.Fprintf(&b, "WATCH %s [id:%s]\n", v.Description, claimID)
			b.WriteString("  → Many reasoning paths depend on this. Consider whether downstream conclusions survive if it's wrong.\n\n")
		case "abandoned_topic":
			fmt.Fprintf(&b, "REVISIT %s\n", v.Description)
			b.WriteString("  → This topic was dropped mid-exploration. Was it resolved, or did something shinier come along?\n\n")
		case "echo_chamber":
			fmt.Fprintf(&b, "PUSHBACK %s\n", v.Description)
			b.WriteString("  → The assistant is validating without challenging. Disagree with something, ask for counter-evidence, or flag an assumption.\n\n")
		}
	}
...
// BuildTopologyPublic is a public wrapper for buildTopology.
func BuildTopologyPublic(claims []types.Claim, edges []types.Edge, project string) types.Topology {
	return *buildTopology(claims, edges, project)
}
