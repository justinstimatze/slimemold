...
func cmdHook() {
	...
	// Consecutive-failure counter for the current cursor position. Even with
	// the core.go fix that stops the cursor from overshooting a capped read,
	// a single ~50-message chunk that itself exceeds the extraction deadline
	// (large tool output, big pasted content) will fail identically on every
	// retry — nothing about the chunk changes between fires. Tracked here so
	// cmdHook can force the cursor past a chunk that's proven unextractable
	// rather than stalling on it forever. See FEEDBACK-extraction-backlog-ratchet.md.
	failCountFile := filepath.Join(logDir, "lastTurnFail-"+hash+".txt")
